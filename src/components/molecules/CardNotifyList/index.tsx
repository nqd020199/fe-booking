...
export const NewsListCard: React.FC<NewsListCardProps> = ({
  ...
  <div className={mapModifiers('o-notifyListCard', isVertical && 'isVertical')}>
    <Link href={href} handleClick={handleClick}>
      {/* {isVertical && (
        <div className="o-notifyListCard_isVertical" />
      )} */}
      <div className="o-notifyListCard_wrapper">
        ...
        <div className="o-notifyListCard_title_date">
          <Typography type="p" modifiers={['12x18', 'dimGray']} content={nameUser} />
          <Typography type="p" modifiers={['12x18', 'dimGray']} content={moment(date).locale('en').format('MM/DD/YYYY hh:mm')} />
        </div>
      </div>
    </Link>
  </div>
...
const CardNotifyList: React.FC<CardNotifyListProps> = ({
  notifyList = [], handleClickDoneAll, title
}) => (
  <div className="o-CardNotifyList">
    <div className="o-CardNotifyList_title">
      ...
      <div className="o-CardNotifyList_title_icon" onClick={handleClickDoneAll}>
        ...
        <div className="o-CardNotifyList_title_title">
          <Typography modifiers={['white', '14x21', '400', 'center']} content="Mark all as read" />
    ...
    <div className="o-CardNotifyList_list">
      {notifyList.length ? (
        notifyList.slice(0).reverse().map((item, idx) => (
          <div className="o-CardNotifyList_list_item" key={`o-CardNotifyList_list${idx.toString()}`}>
            <NewsListCard {...item} />
          </div>
        ))
      ) : <div className="o-CardNotifyList_list-no"><Typography modifiers={['dimGray', '14x21', '400', 'center']} content="No Notify" /></div>}
    </div>
  </div>
);
